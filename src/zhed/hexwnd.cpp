...
{
	return hexeditctl_INTERFACE_VERSION;
}
...
void HexEditorWindow::command(int cmd)
{
	...
	case IDM_CONTEXT:
		...
		{
			if (LONG err = RegDeleteKey(HKEY_CLASSES_ROOT, _T("*\\shell\\Open in zhed\\command"))) //WinNT requires the key to have no subkeys
				hr = HRESULT_FROM_WIN32(err);
			if (LONG err = RegDeleteKey(HKEY_CLASSES_ROOT, _T("*\\shell\\Open in zhed")))
				hr = HRESULT_FROM_WIN32(err);
		}
		else
		{
			hr = CreateShellCommand(_T("*\\shell\\Open in zhed\\command"));
		}
		break;
	case IDM_UNKNOWN:
		...
		{
			HKEY hk;
			if (LONG err = RegDeleteKey(HKEY_CLASSES_ROOT, _T("Unknown\\shell\\Open in zhed\\command"))) //WinNT requires the key to have no subkeys
				hr = HRESULT_FROM_WIN32(err);
			if (LONG err = RegDeleteKey(HKEY_CLASSES_ROOT, _T("Unknown\\shell\\Open in zhed")))
				hr = HRESULT_FROM_WIN32(err);
		...
		{
			hr = CreateShellCommand(_T("Unknown\\shell\\Open in zhed\\command"));
		}
		break;
	case IDM_DEFAULT:
		...
		{
			if (LONG err = RegSetValue(HKEY_CLASSES_ROOT, _T("Unknown\\shell"), REG_SZ, _T("Open in zhed"), 13))
				hr = HRESULT_FROM_WIN32(err);
		}
		break;
		//end
 ...
 * @return 0 if zhed cannot be closed, 1 if we can close zhed.
 */
...
{
	// Is there any data for zhed in the registry?
	HKEY key1;
...
/**
 * @brief Handle file(s) dropped to zhed window.
 */
...
void HexEditorWindow::status_bar_click(bool left)
{
	...
	case 2: //File size part
		...
		//Pabs thinking append, file properties (call shell/zhed) or something else?
...
HGLOBAL HexEditorWindow::RTF_hexdump(int start, int end, SIZE_T* plen)
{
	...
	*/
	...
	{
	case FF_DECORATIVE: s << "\\fdecor ";
		break;
	case FF_DONTCARE:
	case FF_MODERN: s << "\\fmodern ";
		break;
	case FF_ROMAN: s << "\\froman ";
		break;
	case FF_SCRIPT: s << "\\fscript ";
		break;
	case FF_SWISS: s << "\\fswiss ";
		break;
	default: s << "\\fnil ";
		break;
	...
	*/
	...
	{ //Caret is the text colour inverted
		...
		iBkColorValue = ~iBkColorValue;
		iTextColorValue = ~iTextColorValue;
		s <<
	...
	s <<
		...
		"{\\*\\generator zhed v" _CRT_STRINGIZE(ZHED_VERSION_3) ";}\n"
