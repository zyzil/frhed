...
static const TCHAR ZhedLink[] = _T("zhed.lnk");
...
BOOL ShortcutsDlg::OnCommand(HWindow *pDlg, WPARAM w, LPARAM l)
{
	...
	case IDOK:
		{
			...
			//If the key is empty after this kill it (to prevent re-enabling of "Remove zhed")
	...
	case IDC_FIND_AND_FIX:
		//Go through the file system searching for links to zhed.exe and fix them so they point to this exe
	case IDC_UPDATE:
		{
			...
			if (SUCCEEDED(SHGetMalloc(&pMalloc)))
			{
				...
				if (pidl)
				{
					...
					if (SHGetPathFromIDList(pidl, szDir))
					{
						...
						tf.cr = LOWORD(w) == IDC_FIND_AND_FIX;//any zhed.exe if 1 else _pgmptr
						tf.Recurse();//Search
					}
					pMalloc->Free(pidl);
				}
	...
	case IDC_DELETE:
		{
			...
			for( ; ; )
			{
				...
				if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_CURRENT_USER, keyname, 0, KEY_ALL_ACCESS, &hk))
				{
					...
					_tcsncat(delbuf, ZhedLink, RTL_NUMBER_OF(delbuf) - _tcslen(delbuf));
					_tremove(delbuf);
				}
				SHDeleteEmptyKey(HKEY_CURRENT_USER, keyname);
			}
			...
			if (ERROR_SUCCESS == RegOpenKeyEx(HKEY_CURRENT_USER, keyname, 0, KEY_ALL_ACCESS, &hk))
			{
				...
				for (DWORD i = 0 ; ; i++)
				{
					...
					{//Valid dir
						...
						_tcsncat(valbuf, ZhedLink, RTL_NUMBER_OF(valbuf) - _tcslen(valbuf));
