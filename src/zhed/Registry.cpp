 ...
 * @file  Registry.cpp
 *
 * @brief Implementation of zhed registry functions.
 *
 */
#include "precomp.h"
#include "version.h"
...
{
	HKEY key1;
	LONG res = RegOpenKeyEx(HKEY_CLASSES_ROOT, _T("*\\shell\\Open in zhed\\command"), 0, KEY_READ, &key1);
...
BOOL defaultpresent()
{
	...
	return res == ERROR_SUCCESS && _tcscmp(stringval, _T("Open in zhed")) == 0;
}

BOOL unknownpresent()
{
	HKEY key1;
	LONG res = RegOpenKeyEx(HKEY_CLASSES_ROOT, _T("Unknown\\shell\\Open in zhed\\command"), 0, KEY_READ, &key1);
	if (res == ERROR_SUCCESS) //succeeded check if has the required keys & data
	{
		TCHAR stringval[MAX_PATH];
		long len = sizeof stringval;
...
/**
 * @brief Check if zhed settings key already exist.
 */
BOOL zhedpresent()
{
	//Check if zhed\subreleaseno exists
	HKEY hk;
...
/**
 * @brief Check if links key in zhed settings already exist.
 */
BOOL linkspresent()
{
	HKEY hk;
